...
package Game is
   ...
   procedure shipExplosion(monitor : in out gameRecord; i,j : Integer; ship : who);
   function isEnd(b,bp : BoardInterface; i,j : Integer) return Boolean;
   procedure setX(b,bp : in out BoardInterface; i,j : Integer; counter : in out Integer);
   procedure getNext(b,bp : in out BoardInterface; i,j : in out Integer; counter : in out Integer);
end Game;
